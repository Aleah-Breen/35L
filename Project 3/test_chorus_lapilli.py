...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_board_reset(self):
        '''Ensure that after a move is made, refreshing the game resets the board to empty.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()  # Click the first tile to add an "X"
        self.assertTileIs(tiles[0], self.SYMBOL_X)  # Verify "X" was added
        self.driver.refresh()  # Refresh the page to reset the game
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)  # Re-fetch tiles after refresh
        self.assertBoardEmpty(tiles)  # Ensure board is reset to empty

    def test_turn_sequence(self):
        '''Verify that clicking tiles alternates between X and O symbols.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        for i in range(2):
            symbol = self.SYMBOL_X if i % 2 == 0 else self.SYMBOL_O
            tiles[i].click()  # Click the next tile
            self.assertTileIs(tiles[i], symbol)  # Check if tile has correct symbol

    def test_full_board_no_winner(self):
        '''Simulate a full board with no winner and verify all tiles are filled.'''
        moves = [0, 1, 2, 4, 3, 5, 7, 6, 8]  # Pre-determined moves with no winning combination
        symbols = [self.SYMBOL_X if i % 2 == 0 else self.SYMBOL_O for i in range(9)]

        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        for i, move in enumerate(moves):
            tiles[move].click()
            self.assertTileIs(tiles[move], symbols[i])

        # Re-fetch tiles to confirm all are filled
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        for tile in tiles:
            self.assertNotEqual(tile.text.strip(), '', 'Expected full board, found empty tile')
